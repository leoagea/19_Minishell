...
/*   Updated: 2024/08/01 19:46:29 by vdarras          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_simple_builtin(t_cmd *node, t_data *data)
{
	...
	else if (node->next == NULL && node->is_builtin == true && ft_strncmp(node->str[0], "exit", INT_MAX) == 0)
		__exit(data, node);
	else if (node->next == NULL && node->is_builtin == true && ft_strncmp(node->str[0], "export", INT_MAX) == 0)
		_export(data, node);
	else if (node->next == NULL && node->is_builtin == true && ft_strncmp(node->str[0], "unset", INT_MAX) == 0)
		unset(data, node);
	else if (node->next == NULL && node->is_builtin == true && ft_strncmp(node->str[0], "cd", INT_MAX) == 0)
		cd(data);
	return (0);
}